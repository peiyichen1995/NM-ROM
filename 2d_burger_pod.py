...
def plotabs(file_name, uvec, fignum=None, vmax=None, xlabel='',
...
for k, cidx in enumerate(pltgrid):
    ...
    plotabs('figures/' + str(k)+'.png', csol, fignum=5, vmax=vmax, spcls=nplts, spidx=k+1, xlabel=xlab)
...
for k, cidx in enumerate(pltgrid):
    ...
    xlab = '$t={0}$'.format(timegrid[cidx])
    plotabs('figures/pod_' + str(k) + '.png', csol, fignum=6, vmax=vmax, spcls=nplts, spidx=k+1, xlabel=xlab)
    fsol = fullsol[:, cidx]
